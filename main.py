...
def get_disasters():
    ...
    result = []
    for row in rows:
        result.append({"id": row[0], "name": row[1], "type": row[2], "lat": row[3], "lng": row[4]})
    return result
safe_zones = [
...
def add_disaster(disaster: Disaster):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO disasters (name, type, lat, lng) VALUES (?, ?, ?, ?)",
...
def update_disaster(disaster_id: int, disaster: Disaster):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE disasters SET name=?, type=?, lat=?, lng=? WHERE id=?",
...
def delete_disaster(disaster_id: int):
    conn = sqlite3.connect("disasters.db")
    cursor = conn.cursor()
    cursor.execute("DELETE FROM disasters WHERE id=?", (disaster_id,))
    conn.commit()
